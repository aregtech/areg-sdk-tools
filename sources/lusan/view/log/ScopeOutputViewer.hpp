...
private slots:
    ...
    /**
     * \brief   Folds the calls that carry nothing above information, or opens every call.
     * \param   checked  True to fold the quiet calls.
     **/
    void onAutoFoldToggled(bool checked);

    /**
     * \brief   Narrows the view to the rows worth reading, or shows every row again.
     * \param   checked  True to narrow the view.
     **/
    void onInterestingToggled(bool checked);

    /**
     * \brief   Switches the time column between the time of day and the time since the call
     *          the row belongs to was entered.
     * \param   checked  True to count from the entry of the call.
     **/
    void onRelativeTimeToggled(bool checked);
...
private:
    ...
    //!< Builds the fold, the interesting-only and the relative-time controls beside the radios.
    void setupCallControls(void);
...
private:
    ...
    ScopeOutputDelegate*    mStructure; //!< Draws the calls beside the rows and takes the fold clicks.
