...
public:
    ...
    inline const MethodDataSection& getMethods() const;
    inline MethodDataSection& getMethods();
...
public:
    ...
    /**
     * \brief   Resolves a guard address to the guard itself: a transition's `<Guard>`. THE one
     *          resolver, so the editing surface, the undo commands and the validators can never
     *          disagree about which value an address names. Returns nullptr when the ref
     *          addresses nothing or its owning element no longer exists.
     **/
...
private:
    ...
    MethodDataSection    mMethods;       //!< The MethodList section.
...
inline const MethodDataSection& StateMachineData::getMethods() const
{
    return mMethods;
}

inline MethodDataSection& StateMachineData::getMethods()
{
    return mMethods;
}
