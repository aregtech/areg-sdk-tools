...
public:
     ...
     **/
    bool setScopeModel(LogOfflineModel* offlineModel);
...
private:
    ...
    /**
     * \brief   Appends the root node to the model and returns true if succeeded to append.
    ...
    /**
     * \brief   Creates a root scopes out of connected instances.
     */
    void _createRootScopes(void);

    /**
     * \brief   Creates scope nodes from scope information.
     * \param   root        The root node to add scopes to.
     * \param   instId      The ID of the instance to extract scopes.
     **/
    void _createScopeNodes(ScopeRoot& root, ITEM_ID instId);
