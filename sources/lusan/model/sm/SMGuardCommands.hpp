 ...
 * notification for the OWNING element -- the transition, or the state that owns the activity.
 * Because a guard is a small value, every edit is a whole-guard swap: the command captures the
 * previous guard on first redo and restores it on undo. The subtree factories compute the new
 * tree from the current one and hand it to \ref SMSetGuardCommand, so the editor gets one undo
 * step per gesture with no bespoke inverse to maintain.
 **/

/**
 * \class   SMSetGuardCommand
 * \brief   Replaces the addressed guard with a pre-built one (a resolved tree, a draft, or
 ...
 *          ones read the addressed guard's current `ok` tree, apply a deterministic transform to a
...
namespace SMGuardCommands
{
    ...
    SMSetGuardCommand* setGuard(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const SMGuard& guard, const QString& text);

    //!< Installs an `ok` guard around \p tree (takes ownership of \p tree).
    SMSetGuardCommand* setTree(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, SMGuardNode* tree, const QString& text);

    //!< Installs a `draft` guard with the raw \p draftText (takes ownership of \p lastGood).
    SMSetGuardCommand* setDraft(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QString& draftText, SMGuardNode* lastGood, const QString& text);

    //!< Clears to the empty guard (the transition always fires).
    SMSetGuardCommand* clearGuard(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QString& text);

    //!< Replaces argument \p argIndex of the Call node at \p callPath (takes ownership of \p newArg).
    SMSetGuardCommand* replaceArg(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& callPath, int argIndex, SMGuardNode* newArg, const QString& text);
     ...
     **/
    SMSetGuardCommand* setArgByFormal(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& callPath, uint32_t formalId, SMGuardNode* newArg, const QString& text);
     ...
     **/
    SMSetGuardCommand* clearArgByFormal(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& callPath, uint32_t formalId, const QString& text);

    //!< Flips the And/Or combinator of the group node at \p path.
    SMSetGuardCommand* flipCombinator(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& path, const QString& text);

    //!< Negates (wraps in Not) or un-negates (unwraps a Not) the node at \p path.
    SMSetGuardCommand* setNegated(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& path, bool negate, const QString& text);

    //!< Inserts \p clause at \p index into the group at \p groupPath (takes ownership of \p clause).
    SMSetGuardCommand* insertClause(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& groupPath, int index, SMGuardNode* clause, const QString& text);

    //!< Removes the child at \p index from the group at \p groupPath (collapses a 1-child group).
    SMSetGuardCommand* removeClause(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& groupPath, int index, const QString& text);

    //!< Swaps children \p index1 and \p index2 of the group at \p groupPath.
    SMSetGuardCommand* reorderClause(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& groupPath, int index1, int index2, const QString& text);

    //!< Wraps the node at \p path in a new \p groupKind group (And/Or) as its single child.
    SMSetGuardCommand* wrapInGroup(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& path, SMGuardNode::eKind groupKind, const QString& text);

    //!< Replaces the sub-tree at \p path (takes ownership of \p newSubtree).
    SMSetGuardCommand* replaceSubtree(StateMachineData& data, DocModelNotifier& notifier, const SMGuardRef& target, const QList<int>& path, SMGuardNode* newSubtree, const QString& text);
}
